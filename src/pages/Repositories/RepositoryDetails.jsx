...
const RepositoryDetails = () => {
  ...
  // Fetch repository details from your backend
  useEffect(() => {
  ...
  // Fetch GitHub issues & PRs from your backend
  ...
  // Handle lazy-loading repository image
  useEffect(() => {
    if (repository) {
      ...
      if (repository.imageUrl) {
        ...
        setRepoImage(`https://opengraph.githubassets.com/1/${githubInfo.owner}/${githubInfo.repo}`);
      }
    }
  }, [repository, extractGitHubInfo]);
  ...
  if (!repository) return null; // Or a "Not Found" component
...
const GitHubContent = ({ type, data, loading, error, githubInfo, formatDate }) => {
    ...
    if (loading) {
        return <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 text-[#C6FF3D] animate-spin" /></div>;
    }
    if (error) {
        return <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl text-sm"><AlertTriangle className="w-4 h-4 inline mr-2" />{error}</div>;
    }
    return (
...
// Animation Variants (can be moved to a separate file)
