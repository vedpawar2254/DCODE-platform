  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-2">
        ...
        <span className="text-xs sm:text-sm text-gray-400">10 Total</span>
      </div>
        ...
        <div className="relative w-full h-40 mb-8">
          ...
          <div className="flex items-center justify-between h-[100%] z-0 p-[3rem]">
            {prsData.map((pr, index) => {
              const { status, name, date, merged } = pr;
              return (
                <div className="flex flex-col relative w-[10rem]" key={index}>
                  <div
                    className={`absolute max-w-[9rem] flex items-center bg-white/5 px-2 py-1 rounded-full border border-white/10 justify-center right-[-10%] ${index % 2 == 0 ? 'top-[-90%]' : 'bottom-[-90%]'}`}
                  >
                    <h3 className="text-xs line-clamp-1">{`${merged ? '✓' : '✗'} ${name}`}</h3>
                  </div>
                  <h3
                  ...
                  >
                    <path
                      ...
                      stroke="url(#paint0_linear_917_82)"
                      strokeWidth="2"
                    />

                    <defs>
                      <linearGradient
                        id="paint0_linear_917_82"
                        x1="4.76658"
                        y1="66.3737"
                        x2="108.977"
                        y2="9.25827"
                      ...
                      >
                        ...
                        <stop offset="30%" stopColor="#BCDD19" />
            ...
            prsData.slice(0, 6).map((pr, index) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${pr.merged ? 'bg-green-500' : 'bg-red-500'}`}
                  ></div>
                  <span className="text-sm text-white truncate">{pr.name}</span>
                </div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-4 sm:mt-6 pt-4 border-t border-gray-800 gap-3 sm:gap-0">
        <div className="flex flex-wrap gap-3 sm:gap-4 text-xs sm:text-sm">
          <span className="text-gray-400">
            1 <span className="text-white">Opened</span>
          </span>
          <span className="text-gray-400">
            4 <span className="text-purple-400">Merged</span>
          </span>
          <span className="text-gray-400">
            5 <span className="text-red-400">Closed</span>
          </span>
        </div>
