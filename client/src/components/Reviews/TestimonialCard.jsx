...
const TestimonialCard = ({ name, role, text, compact = false }) => (
  ...
  `}>
    ...
    <div className={`flex items-center gap-3 ${compact ? 'mb-3' : 'mb-4'}`}>
      <div className={`flex justify-center items-center ${compact ? 'w-10 h-10' : 'w-12 h-12'} rounded-full bg-[#01FF801F]`}>
        <div className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} rounded-full bg-[#01FF804D]`} />
      </div>
      <div>
        <h3 className={`font-semibold ${compact ? 'text-sm' : 'text-base'}`}>{name}</h3>
        <p className={`${compact ? 'text-[10px]' : 'text-xs'} text-[#D5D5D5B3]`}>{role}</p>
      </div>
    </div>

    {/* Testimonial text */}
    <p className={`${compact ? 'text-xs' : 'text-sm'} text-[#D5D5D5] ${compact ? 'mb-3' : 'mb-4'} leading-relaxed`}>
