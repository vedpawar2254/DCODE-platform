import { useState } from 'react';
import { FeatureCards } from './FeatureCards';
import playbutton from '../../../assets/playbutton.svg';
import accelerate from '../../../assets/accelerate.svg';

const Bennefits = () => {
  const [isVideoOpen, setIsVideoOpen] = useState(false);
  return (
    <>
      <section className="relative w-full px-6 md:px-16 bg-black text-center overflow-hidden min-h-screen flex flex-col justify-center items-center">
        {/* Section intro */}
        <div className="mb-16 -mt-8 max-w-3xl mx-auto">
          <h3 className="text-green-500 text-sm tracking-widest mb-2">
            WHY CHOOSE
          </h3>
          <h2 className="text-4xl md:text-5xl font-bold text-white">
            Experience <span className="text-[#BCDD19]">DCODE</span>
          </h2>
          <p className="text-gray-400 mt-4">
            DCODE is an open-source initiative by Dev Club that empowers
            students to contribute to real-world projects and build
            industry-level skills. It fosters collaboration, learning, and a
            lasting culture of open-source development.
          </p>
        </div>

        <div className="relative flex flex-col md:flex-row md:flex-wrap md:justify-between items-center gap-12 mx-16 w-full">
          {!isVideoOpen ? (
            <>
              {/* Left column */}
              <div
                className={`flex flex-col space-y-12 md:space-y-20 transform transition-all duration-700 ease-in-out ${
                  isVideoOpen
                    ? '-translate-x-[200%] opacity-0'
                    : 'translate-x-0 opacity-100'
                }`}
              >
                <FeatureCards
                  icon="📈"
                  title="Accelerate Growth"
                  description="Build meaningful projects that advance your career trajectory"
                />
                <FeatureCards
                  icon="🎯"
                  title="Unlock Potential"
                  description="Maximize your impact with cutting-edge development tools"
                />
              </div>

              {/* Center circle */}
              <div
                className={`relative flex items-center justify-center transition-all duration-700 ease-in-out ${
                  isVideoOpen ? 'scale-50 opacity-0' : 'scale-100 opacity-100'
                }`}
              >
                <div className="w-64 h-64 rounded-full bg-gradient-to-b from-[#5A6525] to-[#333D00] flex items-center justify-center">
                  <div className="w-56 h-56 rounded-full flex items-center justify-center border-1 border-[#37CD5A]">
                    <button
                      onClick={() => setIsVideoOpen(true)}
                      className="absolute top-1/2 left-1/2 w-[100px] h-[100px] -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center hover:scale-105 transition"
                      style={{
                        background:
                          'radial-gradient(circle, #37CD5A 0%, transparent 100%)'
                      }}
                    >
                      <img
                        src={playbutton}
                        alt="play button"
                        className="w-8 h-8"
                      />
                    </button>
                  </div>
                </div>
              </div>

              {/* Bubbles */}
              <div
                className={`absolute w-8 h-8 rounded-full transition-all duration-700 ease-in-out ${
                  isVideoOpen
                    ? 'translate-y-[-200%] opacity-0'
                    : 'translate-y-0 opacity-100'
                }`}
                style={{
                  top: '-40px',
                  left: '60%',
                  transform: 'translateX(-50%)',
                  background:
                    'radial-gradient(circle, #37CD5A 0%, transparent 100%)'
                }}
              ></div>

              <div
                className={`absolute w-6 h-6 rounded-full transition-all duration-700 ease-in-out ${
                  isVideoOpen
                    ? 'translate-y-[200%] opacity-0'
                    : 'translate-y-0 opacity-100'
                }`}
                style={{
                  bottom: '-40px',
                  right: '60%',
                  background:
                    'radial-gradient(circle, #37CD5A 0%, transparent 100%)'
                }}
              ></div>

              {/* Right column */}
              <div
                className={`flex flex-col space-y-12 md:space-y-20 transform transition-all duration-700 ease-in-out ${
                  isVideoOpen
                    ? 'translate-x-[200%] opacity-0'
                    : 'translate-x-0 opacity-100'
                }`}
              >
                <FeatureCards
                  icon="⭐"
                  title="Stand Apart"
                  description="Distinguish yourself in the competitive development landscape"
                />
                <FeatureCards
                  icon="🌐"
                  title="Collaborate Globally"
                  description="Work with diverse peers and mentors from across the tech ecosystem"
                />
              </div>
            </>
          ) : (
            <div className="w-full flex justify-center items-center transition-all duration-7000 ease-in-out">
              <video
                src="https://www.w3schools.com/html/mov_bbb.mp4"
                controls
                autoPlay
                className="rounded-xl shadow-xl w-full max-w-4xl"
              />
            </div>
          )}
        </div>
      </section>
    </>
  );
};

export default Bennefits;
